...
function Widget(completion, data) {
  ...
  completions.forEach((completion, index) => {
    ...
    let className =
      HINT_ELEMENT_CLASS +
      (index !== this.selectedHint ? "" : " " + ACTIVE_HINT_ELEMENT_CLASS);
  ...
  if (container !== ownerDocument.body) {
    ...
    const isContainerPositioned = ["absolute", "relative", "fixed"].includes(
  ...
  cm.on(
    ...
    (this.onScroll = function () {
      ...
      hints.style.left = left + startScroll.left - currentScroll.left + "px";
    })
  );
