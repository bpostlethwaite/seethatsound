...
export function editorEvalCode(
  csound,
  csoundStatus,
  documentType,
  printToConsole,
  ...
  if (csoundStatus !== "playing") {
    ...
    if (!blockEval) {
      ...
      if (result) {
        ...
        markerCallback = (hasError) => {
          const textMarker = editorReference.markText(result.from, result.to, {
            className: hasError ? "blinkEvalError" : "blinkEval",
          });
          setTimeout(() => textMarker.clear(), 200);
        };
    ...
    if (documentType === "orc" || documentType === "udo") {
      ...
      printToConsole && printToConsole("Can't evaluate non-csound documents!");
    }
  }
}

export function uncommentLine(line) {
  let uncommentedLine = line.split(";");
