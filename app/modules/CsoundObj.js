...
export default class CsoundObj {
     ...
     */
    constructor() {
        this.audioContext = window.CSOUND_AUDIO_CONTEXT;
    ...
    */
    ...
    evaluateCode(codeString) {
        return this.node.evaluateCode(codeString);
    }
    ...
    enableAudioInput(audioInputCallback) {
        ...
        if (
            ...
            let onSuccess = function(stream) {
                that.microphoneNode = window.CSOUND_AUDIO_CONTEXT.createMediaStreamSource(
                    stream
                );
    ...
    static importScripts(script_base = "./") {
        return new Promise(resolve => {
            csound_load_script(script_base + CSOUND_NODE_SCRIPT, () => {
                if (window.CSOUND_AUDIO_CONTEXT.hasAudioWorklet)
                    window.CSOUND_AUDIO_CONTEXT.factory = window.CsoundNodeFactory;
                else
                    window.CSOUND_AUDIO_CONTEXT.factory = window.CsoundScriptProcessorNodeFactory;
                window.CSOUND_AUDIO_CONTEXT.factory
                    .importScripts(script_base)
                    .then(() => {
                        resolve();
                    });
            });
    ...
    static createNode(inputChannelCount = 1, outputChannelCount = 2) {
        return window.CSOUND_AUDIO_CONTEXT.factory.createNode(
            inputChannelCount,
            outputChannelCount
        );
    }
